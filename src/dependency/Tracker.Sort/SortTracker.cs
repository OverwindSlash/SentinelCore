...
using Tracker.Sort.Kalman;

namespace Tracker.Sort
{
    public class SortTracker : ITracker, IObjectTracker
    {
        ...
        {
            IEnumerable<RectangleF> bboxs = from obj in detectedObjects
            ...
            foreach (var detectedObject in detectedObjects)
            {
                ...
                RectangleF roi = new RectangleF(bbox.X, bbox.Y, bbox.Width, bbox.Height);
                foreach (Track track in tracks)
                {
                    if (track.History.Contains(roi))
                    {
                        bbox.TrackingId = track.TrackId;
                    }
                }
            }
        }
